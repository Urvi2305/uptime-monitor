...
const URL_WITH_LATEST_CHECK_COLUMNS = `
...
async function listUrls(req, res) {
  ...
  const [{ rows: countRows }, { rows }] = await Promise.all([
    ...
    pool.query(
      `SELECT ${URL_WITH_LATEST_CHECK_COLUMNS}
       FROM urls u
       ${LATEST_CHECK_JOIN}
...
async function getUrlById(req, res) {
  ...
  sendSuccess(res, { statusCode: STATUS.OK, message: 'URL fetched successfully', data: rows[0] });
}
...
async function getUrlChecks(req, res) {
  ...
  const [{ rows: countRows }, { rows }] = await Promise.all([
    pool.query('SELECT COUNT(*)::int AS total FROM checks WHERE url_id = $1', [id]),
    pool.query(
...
module.exports = { createUrl, listUrls, getUrlById, getUrlChecks, deleteUrl };
